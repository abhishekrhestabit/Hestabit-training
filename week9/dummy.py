...
def execute_python_script(code: str) -> str:
    ...
    try:
        # Write to temp file for clean tracebacks
        ...
        if result.returncode == 0:
            return stdout if stdout else "Script ran successfully (no output)."
        else:
...
YOUR TOOLS:
  execute_python_script(code)   → runs Python in a subprocess, returns stdout
                                  missing packages are auto-installed before running

RULES:
  1. Always call execute_python_script() — never show code without running it.
  2. Write COMPLETE, runnable code (imports included).
  3. Use print() for every result — that is the only thing captured.
