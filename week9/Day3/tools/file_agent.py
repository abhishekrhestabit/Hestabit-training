...
def _replace_file(destination: Path, *, text: str | None = None, source: Path | None = None) -> None:
    ...
    if source:
        ...
        temp.write_text(text or "", encoding="utf-8")
...
async def list_files(
    ...
    inside_ignored = (
        base.is_relative_to(PROJECT_ROOT)
        and any(part in IGNORED_DIRS for part in base.relative_to(PROJECT_ROOT).parts)
    )

    def _not_ignored(p: Path) -> bool:
        return inside_ignored or not any(part in IGNORED_DIRS for part in p.parts)

    matches = sorted(p for p in base.rglob(pattern) if p.is_file() and _not_ignored(p))

    if not matches:
        is_literal = not any(c in pattern for c in "*?[")
        if is_literal:
            return f"ERROR: File not found: {base / pattern}"
...
def _summarize_csv(path: str, *, rows: int = 5, detailed: bool = False) -> str:
    ...
    if not detailed:
        dtype_lines = [f"- {c}: {t}" for c, t in df.dtypes.items()]
        lines += ["Dtypes:", *dtype_lines, "", "Sample rows:", df.head(rows).to_string(index=False)]
        return "\n".join(lines)
    ...
    if len(num_cols):
        num_lines = [
            f"- {c}: min={df[c].min()}, max={df[c].max()}, mean={df[c].mean():.2f}, median={df[c].median():.2f}"
            for c in num_cols
        ]
        lines += ["Numeric summary:", *num_lines]

    cat_cols = df.select_dtypes(exclude="number").columns
    if len(cat_cols):
        cat_lines = [
            f"- {c}: {', '.join(f'{i}={v}' for i, v in df[c].value_counts().head(5).items())}"
            for c in cat_cols
        ]
        lines += ["Categorical summary:", *cat_lines]
...
async def copy_file_to_workspace(
    ...
    # If src and dst are the same , resolve to project root
    if dst == src:
